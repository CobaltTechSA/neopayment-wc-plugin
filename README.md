...
2. Upload the entire `class-cbopaga-payment-gateway` folder into:
...
1. **Extract strings to a `.pot` file**  
   ```bash
   wp i18n make-pot . i18n/class-cbopaga-payment-gateway.pot --slug=class-cbopaga-payment-gateway
   ```
...
2. **Initialize a new locale** (example: Spanish – `es_ES`)  
   ...
   msginit \
     --no-translator \
     --input=i18n/class-cbopaga-payment-gateway.pot \
     --locale=es_ES \
     --output-file=i18n/class-cbopaga-payment-gateway-es_ES.po
   ```

3. **Translate strings**  
   - Open `i18n/class-cbopaga-payment-gateway-es_ES.po`  
...
4. **Compile the `.mo` file**  
   ```bash
   msgfmt \
     i18n/class-cbopaga-payment-gateway-es_ES.po \
     -o i18n/class-cbopaga-payment-gateway-es_ES.mo
   ```

5. **Generate JSON for React/JS blocks**  
   ```bash
   wp i18n make-json i18n i18n --domain=class-cbopaga-payment-gateway
   ```
   - **Input**: All `.pot` files inside `i18n/`  
   - **Output**: `class-cbopaga-payment-gateway-<locale>.json` files in `i18n/`
