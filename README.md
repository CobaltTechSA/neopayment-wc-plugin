...
2. Upload the entire `neopayment` folder into:
...
1. **Extract strings to a `.pot` file**  
   ```bash
   wp i18n make-pot . i18n/neopayment.pot --slug=neopayment
   ```
...
2. **Initialize a new locale** (example: Spanish – `es_ES`)  
   ...
   msginit \
     --no-translator \
     --input=i18n/neopayment.pot \
     --locale=es_ES \
     --output-file=i18n/neopayment-es_ES.po
   ```

3. **Translate strings**  
   - Open `i18n/neopayment-es_ES.po`  
...
4. **Compile the `.mo` file**  
   ```bash
   msgfmt \
     i18n/neopayment-es_ES.po \
     -o i18n/neopayment-es_ES.mo
   ```

5. **Generate JSON for React/JS blocks**  
   ```bash
   wp i18n make-json i18n i18n --domain=neopayment
   ```
   - **Input**: All `.pot` files inside `i18n/`  
   - **Output**: `neopayment-<locale>.json` files in `i18n/`
