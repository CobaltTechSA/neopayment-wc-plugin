...
require_once 'cobalt-bank-operations-payment-gateway-helpers.php';
 ...
 */
class COBALT_BANK_OPERATIONS_PAYMENT_GATEWAY_Standard_Gateway extends WC_Payment_Gateway
{
	...
	public function __construct()
	{
		...
		$this->id                 = COBALT_BANK_OPERATIONS_PAYMENT_GATEWAY_Constants::COBALT_BANK_OPERATIONS_PAYMENT_GATEWAY_STANDARD_GATEWAY_ID; // payment gateway plugin ID.
		...
		add_action(
			...
			function () {
				wp_nonce_field('cobalt_bank_operations_payment_gateway_standard_save_settings', 'cobalt_bank_operations_payment_gateway_standard_nonce');
			}
		);
	}
	...
	{
		if (
			! isset($_POST['cobalt_bank_operations_payment_gateway_standard_nonce']) ||
			! wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['cobalt_bank_operations_payment_gateway_standard_nonce'])), 'cobalt_bank_operations_payment_gateway_standard_save_settings')
		) {
...
?>
	...
	public function register_plugin_scripts()
	{
		...
		$ver  = COBALT_BANK_OPERATIONS_PAYMENT_GATEWAY_Constants::COBALT_BANK_OPERATIONS_PAYMENT_GATEWAY_PLUGIN_VERSION;
		...
		wp_enqueue_script(
			'cobalt-bank-operations-payment-gateway-standard-payment',
			$base . 'cobalt-bank-operations-payment-gateway-script.js',
			array('jquery'),
			$ver,
			true
		);
		...
		wp_enqueue_script(
			...
			$base . 'cobalt-bank-operations-payment-gateway-3ds-popup.js',
	...
	public function validate_fields()
	{
		...
		if (! cobalt_bank_operations_payment_gateway_is_valid_luhn($card_number)) {
			wc_add_notice(__('Invalid card number', 'cobalt-bank-operations-payment-gateway'), 'error');
			$valid = false;
		}

		$card_expiry = str_replace(' ', '', $card_expiry);
		if (! cobalt_bank_operations_payment_gateway_is_valid_expiry_date($card_expiry)) {
			wc_add_notice(__('Invalid expiry date', 'cobalt-bank-operations-payment-gateway'), 'error');
			$valid = false;
		}

		if (! cobalt_bank_operations_payment_gateway_is_valid_card_holder($card_holder)) {
			wc_add_notice(__('Invalid card holder', 'cobalt-bank-operations-payment-gateway'), 'error');
			$valid = false;
		}

		if (! cobalt_bank_operations_payment_gateway_is_valid_cvv($card_cvv)) {
	...
	public function process_refund($order_id, $amount = 0, $reason = '')
	{
		if (! isset($_REQUEST['security']) || ! check_ajax_referer('order-item', 'security', false)) {
			COBALT_BANK_OPERATIONS_PAYMENT_GATEWAY_Log::debug('Refund rechazado: nonce inválido o ausente');
			return new WP_Error('invalid_nonce', __('Unauthorized action.', 'cobalt-bank-operations-payment-gateway'));
		}

		$cobalt_bank_operations_payment_gateway_client = new COBALT_BANK_OPERATIONS_PAYMENT_GATEWAY_Client(
			$this->api_url,
			$this->api_client_id,
		...
		COBALT_BANK_OPERATIONS_PAYMENT_GATEWAY_Log::debug("process_refund: order_id={$order_id}, amount={$amount}, reason={$reason}");
		$order = wc_get_order($order_id);

		if (! $order) {
			return new WP_Error('invalid_order', 'Invalid order ID');
		}
		$txn = $order->get_meta('cobalt_bank_operations_payment_gateway_transaction_id');
		if (! $txn) {
			return new WP_Error('no_transaction_id', 'No transaction ID found for this order');
		}

		try {
			$data = $cobalt_bank_operations_payment_gateway_client->refund($txn, intval($amount * 100));
		} catch (COBALT_BANK_OPERATIONS_PAYMENT_GATEWAY_Exception $e) {
			COBALT_BANK_OPERATIONS_PAYMENT_GATEWAY_Log::debug('Error processing refund: ' . $e->getMessage());
			return new WP_Error('cobalt_bank_operations_payment_gateway_refund_error', $e->getMessage());
		}
		...
		if ( in_array( $status, $success_status, true ) ) {
			$order->add_order_note(
				sprintf(
					'Reembolso de %s aprobado vía CBO (refund_id %s). Motivo: %s',
					wc_price($amount),
			...
			if (! empty($data['identifier'])) {
				$order->update_meta_data('cobalt_bank_operations_payment_gateway_refund_id', $data['identifier']);
				$order->save();
			}
			return true;
		} else {
			$order->add_order_note(
				sprintf(
					'Intento de reembolso fallido. Estado devuelto por CBO: %s (código %s)',
					$status,
			...
			return new WP_Error(
				'refund_denied',
				sprintf(
					__('Reembolso denegado (estado: %s).', 'cobalt-bank-operations-payment-gateway'),
					$status
				)
			);
		}
	...
	public function process_payment($order_id)
	{
		...
		COBALT_BANK_OPERATIONS_PAYMENT_GATEWAY_Log::debug('process_payment: ' . $order_id);
		$order = wc_get_order($order_id);
		...
		try {
			...
			$cobalt_bank_operations_payment_gateway_is_block = ! empty($body['payment_data']);

			// if the request is from a block-based checkout, we need to handle it differently.
			if ($cobalt_bank_operations_payment_gateway_is_block) {
				COBALT_BANK_OPERATIONS_PAYMENT_GATEWAY_Log::debug('Origin: Checkout Based Blocks');
				...
				COBALT_BANK_OPERATIONS_PAYMENT_GATEWAY_Log::debug(' data: ' . wp_json_encode($data));
				...
				$three_ds_params = array(
					...
					'billAddrCountry'          => $this->cobalt_bank_operations_payment_gateway_get_iso_alpha3_cc($billing['country'] ?? $order->get_billing_country()),
					'billAddrCity'             => $billing['city'] ?? $order->get_billing_city(),
					'billAddrState'            => cobalt_bank_operations_payment_gateway_parse_state($billing['state'] ?? $order->get_billing_state()),
					'billAddrLine1'            => $billing['address_1'] ?? $order->get_billing_address_1(),
					'billAddrLine2'            => 'none',
					'billAddrPostCode'         => $billing['postcode'] ?? $order->get_billing_postcode(),
					'shipAddrCountry'          => $this->cobalt_bank_operations_payment_gateway_get_iso_alpha3_cc($shipping['country'] ?? $order->get_shipping_country()),
					'shipAddrCity'             => $shipping['city'] ?? $order->get_shipping_city(),
					'shipAddrState'            => cobalt_bank_operations_payment_gateway_parse_state($shipping['state'] ?? $order->get_shipping_state()),
				...
				COBALT_BANK_OPERATIONS_PAYMENT_GATEWAY_Log::debug('Origin: Classic Checkout');
				...
				$three_ds_params = $this->cobalt_bank_operations_payment_gateway_get_3ds_params();
			}
			COBALT_BANK_OPERATIONS_PAYMENT_GATEWAY_Log::debug('three_ds_params=' . wp_json_encode($three_ds_params));

			$transaction = $cobalt_bank_operations_payment_gateway_client->sale($order, $card_number, $card_expiry, $card_cvc, $card_holder, $three_ds_params);
			COBALT_BANK_OPERATIONS_PAYMENT_GATEWAY_Log::debug('Checkout data: ' . wp_json_encode($transaction));
			...
			if ('authenticating' === ($transaction['status'] ?? '')) {
				return array(
					...
					'redirect'           => '',
					'cobalt_bank_operations_payment_gateway_callback_url'       => $callback,
				);
	 ...
	 * @return array
	 */
	private function cobalt_bank_operations_payment_gateway_get_3ds_params()
	{
		...
		$billing_iso3                       = ('' !== $billing_country_raw) ? $this->cobalt_bank_operations_payment_gateway_get_iso_alpha3_cc($billing_country_raw) : '';
		...
		$three_ds_params['billAddrState'] = ('' !== $billing_state) ? cobalt_bank_operations_payment_gateway_parse_state($billing_state) : 'DIG';
		...
		$shipping_iso3                      = ('' !== $shipping_country_raw) ? $this->cobalt_bank_operations_payment_gateway_get_iso_alpha3_cc($shipping_country_raw) : '';
		...
		$three_ds_params['shipAddrState'] = ('' !== $shipping_state) ? cobalt_bank_operations_payment_gateway_parse_state($shipping_state) : 'DIG';
	...
	private function validate_payment($transaction)
	{
		...
		$order->update_meta_data('cobalt_bank_operations_payment_gateway_bank_code', $transaction['response_code']);
		$order->update_meta_data('cobalt_bank_operations_payment_gateway_transaction_id', $transaction['identifier']);
		$order->update_meta_data('cobalt_bank_operations_payment_gateway_bank_authorization', $transaction['authorization_number']);
	...
	public function cobalt_bank_operations_payment_gateway_callback_url()
	{
		...
		COBALT_BANK_OPERATIONS_PAYMENT_GATEWAY_Log::debug("cobalt_bank_operations_payment_gateway_callback_url: order_id=$order_id, target=$target");
			...
			document.addEventListener("DOMContentLoaded", function() {
				if (window.opener && !window.opener.closed) {
					window.opener.postMessage({
						...
						source: "cobalt_bank_operations_payment_gateway_3ds_handler"
					}, "' . esc_url(home_url('/')) . '");
...
<?php
	...
	public function cobalt_bank_operations_payment_gateway_webhook()
	{
		...
		if (! is_array($data)) {
			COBALT_BANK_OPERATIONS_PAYMENT_GATEWAY_Log::debug('Webhook error: input no es array válido');
			status_header(400);
			exit;
		}
		...
		COBALT_BANK_OPERATIONS_PAYMENT_GATEWAY_Log::debug('Webhook recibido: ' . wp_json_encode($data));
		...
		try {
			...
			COBALT_BANK_OPERATIONS_PAYMENT_GATEWAY_Log::debug("Error en webhook. TID: $tid - " . $e->getMessage());
			status_header(400);
		}
		exit;
	}
	 ...
	 */
	public function cobalt_bank_operations_payment_gateway_get_iso_alpha3_cc($country)
	{
		return COBALT_BANK_OPERATIONS_PAYMENT_GATEWAY_Constants::COBALT_BANK_OPERATIONS_PAYMENT_GATEWAY_PAYMENT_COUNTRIES[$country] ?? $country;
	}
