...
require_once 'nbo-payment-gateway-helpers.php';
require_once 'class-nbo-payment-gateway-cc.php';

/**
 * Handles WooCommerce Standard integration for the payment gateway.
 */
class NBO_PAYMENT_GATEWAY_Standard_Gateway extends WC_Payment_Gateway
{
	...
	public function __construct()
	{
		...
		$this->id                 = NBO_PAYMENT_GATEWAY_Constants::NBO_PAYMENT_GATEWAY_STANDARD_GATEWAY_ID; // payment gateway plugin ID.
		...
		add_action('woocommerce_api_' . $this->id, array($this, 'nbo_payment_gateway_webhook'));

		// URL OK y KO.
		add_action('woocommerce_api_' . $this->id . '_status', array($this, 'nbo_payment_gateway_callback_url'));
		...
		add_action(
			'woocommerce_admin_field_nbo_payment_gateway_nonce',
			function () {
				wp_nonce_field('nbo_payment_gateway_standard_save_settings', 'nbo_payment_gateway_standard_nonce');
			}
		);
	}
	...
	{
		if (
			! isset($_POST['nbo_payment_gateway_standard_nonce']) ||
			! wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['nbo_payment_gateway_standard_nonce'])), 'nbo_payment_gateway_standard_save_settings')
		) {
	...
	public function get_icon()
	{
		...
		$icons = array(
			sprintf(
				...
				esc_attr('nbo-payment-gateway-icon'),
...
?>
	...
	public function register_plugin_scripts()
	{
		...
		$ver  = NBO_PAYMENT_GATEWAY_Constants::NBO_PAYMENT_GATEWAY_PLUGIN_VERSION;
		...
		wp_enqueue_script(
			'nbo-payment-gateway-standard-payment',
			$base . 'nbo-payment-gateway-script.js',
			array('jquery'),
			$ver,
			true
		);
		...
		wp_enqueue_script(
			...
			$base . 'nbo-payment-gateway-3ds-popup.js',
	...
	public function process_refund($order_id, $amount = 0, $reason = '')
	{
		if (! isset($_REQUEST['security']) || ! check_ajax_referer('order-item', 'security', false)) {
			NBO_PAYMENT_GATEWAY_Log::debug('Refund rechazado: nonce inválido o ausente');
			return new WP_Error('invalid_nonce', __('Unauthorized action.', 'nbo-payment-gateway'));
		}

		$nbo_payment_gateway_client = new NBO_PAYMENT_GATEWAY_Client(
			$this->api_url,
			$this->api_client_id,
		...
		NBO_PAYMENT_GATEWAY_Log::debug("process_refund: order_id={$order_id}, amount={$amount}, reason={$reason}");
		$order = wc_get_order($order_id);

		if (! $order) {
			return new WP_Error('invalid_order', 'Invalid order ID');
		}
		$txn = $order->get_meta('nbo_payment_gateway_transaction_id');
		if (! $txn) {
			return new WP_Error('no_transaction_id', 'No transaction ID found for this order');
		}

		try {
			$data = $nbo_payment_gateway_client->refund($txn, intval($amount * 100));
		} catch (NBO_PAYMENT_GATEWAY_Exception $e) {
			NBO_PAYMENT_GATEWAY_Log::debug('Error processing refund: ' . $e->getMessage());
			return new WP_Error('nbo_payment_gateway_refund_error', $e->getMessage());
		}
		...
		if ( in_array( $status, $success_status, true ) ) {
			$order->add_order_note(
				sprintf(
					'Reembolso de %s aprobado vía NBO (refund_id %s). Motivo: %s',
					wc_price($amount),
			...
			if (! empty($data['identifier'])) {
				$order->update_meta_data('nbo_payment_gateway_refund_id', $data['identifier']);
				$order->save();
			}
			return true;
		} else {
			$order->add_order_note(
				sprintf(
					'Intento de reembolso fallido. Estado devuelto por NBO: %s (código %s)',
					$status,
	...
	public function process_payment($order_id)
	{
		...
		NBO_PAYMENT_GATEWAY_Log::debug('process_payment: ' . $order_id);
		$order = wc_get_order($order_id);

		$nbo_payment_gateway_client = new NBO_PAYMENT_GATEWAY_Client($this->api_url, $this->api_client_id, $this->api_client_secret);
		try {
			// detect if the request is from a block-based checkout or classic checkout.
			$raw_input = file_get_contents('php://input');
			$body      = json_decode($raw_input, true);
			...
			$nbo_payment_gateway_is_block = ! empty($body['payment_data']);

			// if the request is from a block-based checkout, we need to handle it differently.
			if ($nbo_payment_gateway_is_block) {
				NBO_PAYMENT_GATEWAY_Log::debug('Origin: Checkout Based Blocks');
				...
				NBO_PAYMENT_GATEWAY_Log::debug(' data: ' . wp_json_encode($data));
				...
				NBO_PAYMENT_GATEWAY_Log::debug('Origin: Classic Checkout');
				...
				$three_ds_params = $this->nbo_payment_gateway_get_3ds_params();
			}
			NBO_PAYMENT_GATEWAY_Log::debug('three_ds_params=' . wp_json_encode($three_ds_params));

			$transaction = $nbo_payment_gateway_client->sale($order, $card_number, $card_expiry, $card_cvc, $card_holder, $three_ds_params);
			NBO_PAYMENT_GATEWAY_Log::debug('Checkout data: ' . wp_json_encode($transaction));
			...
			if ('authenticating' === ($transaction['status'] ?? '')) {
				return array(
					...
					'redirect'           => '',
					'nbo_payment_gateway_callback_url'       => $callback,
				);
			...
			if (! $e->isSuccessResponse()) {
				NBO_PAYMENT_GATEWAY_Log::debug($e->getMessage() . ' - ' . wp_json_encode($e->getResponse()));
	 ...
	 * @return array
	 */
	private function nbo_payment_gateway_get_3ds_params()
	{
		...
		$billing_iso3                       = ('' !== $billing_country_raw) ? $this->nbo_payment_gateway_get_iso_alpha3_cc($billing_country_raw) : '';
		...
		$shipping_iso3                      = ('' !== $shipping_country_raw) ? $this->nbo_payment_gateway_get_iso_alpha3_cc($shipping_country_raw) : '';
	...
	private function validate_payment($transaction)
	{
		...
		$order->update_meta_data('nbo_payment_gateway_bank_code', $transaction['response_code']);
		$order->update_meta_data('nbo_payment_gateway_transaction_id', $transaction['identifier']);
		$order->update_meta_data('nbo_payment_gateway_bank_authorization', $transaction['authorization_number']);
	 ...
	 */
	public function nbo_payment_gateway_callback_url()
	{
		...
		NBO_PAYMENT_GATEWAY_Log::debug("nbo_payment_gateway_callback_url: order_id=$order_id, target=$target");
			...
			document.addEventListener("DOMContentLoaded", function() {
				if (window.opener && !window.opener.closed) {
					window.opener.postMessage({
						...
						source: "nbo_payment_gateway_3ds_handler"
					}, "' . esc_url(home_url('/')) . '");
...
<?php
	 ...
	 */
	public function nbo_payment_gateway_webhook()
	{
		$raw_input = file_get_contents('php://input');
		$data      = json_decode($raw_input, true);

		if (! is_array($data)) {
			NBO_PAYMENT_GATEWAY_Log::debug('Webhook error: input no es array válido');
			status_header(400);
			exit;
		}
		...
		NBO_PAYMENT_GATEWAY_Log::debug('Webhook recibido: ' . wp_json_encode($data));
		...
		try {
			...
			NBO_PAYMENT_GATEWAY_Log::debug("Error en webhook. TID: $tid - " . $e->getMessage());
			status_header(400);
		}
		exit;
	}
	 ...
	 */
	public function nbo_payment_gateway_get_iso_alpha3_cc($country)
	{
		return NBO_PAYMENT_GATEWAY_Constants::NBO_PAYMENT_GATEWAY_PAYMENT_COUNTRIES[$country] ?? $country;
	}
