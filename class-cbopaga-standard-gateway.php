...
require_once 'cbopaga-helpers.php';
require_once 'class-cbopaga-payment-gateway-cc.php';
...
class CBOPAGA_Standard_Gateway extends WC_Payment_Gateway {
	...
	public function __construct() {
		...
		$this->method_description = __( 'Acceptance of payments with Visa / Mastercard', 'class-cbopaga-payment-gateway' ); // will be displayed on the options page.
	...
	public function get_icon() {
		...
		$icons = array(
			sprintf(
				...
				esc_attr__( 'Visa', 'class-cbopaga-payment-gateway' )
			),
			sprintf(
				'<img class="%s" src="%s" alt="%s" />',
				esc_attr( 'cbo-gateway-icon' ),
				esc_url( WC_HTTPS::force_https_url( $path . 'assets/images/mastercard.svg' ) ),
				esc_attr__( 'Mastercard', 'class-cbopaga-payment-gateway' )
			),
		);
	...
	public function register_plugin_scripts() {
		...
		wp_enqueue_script(
			...
			array( 'jquery', 'wc-checkout', 'cbopaga-sweetalert' ),
			$ver,
			true
		);
	...
	public function validate_fields() {
		...
		if ( isset( $_POST[ $this->id . '_nonce' ] ) &&
			...
			wc_add_notice( __( 'Security check failed. Please try again.', 'class-cbopaga-payment-gateway' ), 'error' );
			return false;
		}
		...
		if ( ! cbopaga_is_valid_luhn( $card_number ) ) {
			wc_add_notice( __( 'Invalid card number', 'class-cbopaga-payment-gateway' ), 'error' );
			$valid = false;
		}

		$card_expiry = str_replace( ' ', '', $card_expiry );
		if ( ! cbopaga_is_valid_expiry_date( $card_expiry ) ) {
			wc_add_notice( __( 'Invalid expiry date', 'class-cbopaga-payment-gateway' ), 'error' );
			$valid = false;
		}

		if ( ! cbopaga_is_valid_card_holder( $card_holder ) ) {
			wc_add_notice( __( 'Invalid card holder', 'class-cbopaga-payment-gateway' ), 'error' );
			$valid = false;
		}

		if ( ! cbopaga_is_valid_cvv( $card_cvv ) ) {
			wc_add_notice( __( 'Invalid card code (CVV)', 'class-cbopaga-payment-gateway' ), 'error' );
			$valid = false;
		}
	...
	public function process_refund( $order_id, $amount = 0, $reason = '' ) {
		if ( ! isset( $_REQUEST['security'] ) || ! check_ajax_referer( 'order-item', 'security', false ) ) {
			...
			return new WP_Error( 'invalid_nonce', __( 'Unauthorized action.', 'class-cbopaga-payment-gateway' ) );
		}

		$cbo_client = new CBOPAGA_Client(
			$this->api_url,
			$this->api_client_id,
		...
		try {
			$data = $cbo_client->refund( $txn, intval( $amount * 100 ) );
	...
	public function process_payment( $order_id ) {
		...
		try {
			...
			if ( $cbopaga_is_block ) {
				...
				CBOPAGA_Log::debug( ' data: ' . wp_json_encode( $data ) );
				...
				$three_ds_params = array(
					...
					'billAddrState'            => cbopaga_parse_state( $billing['state'] ?? $order->get_billing_state() ),
					...
					'shipAddrState'            => cbopaga_parse_state( $shipping['state'] ?? $order->get_shipping_state() ),
			...
			CBOPAGA_Log::debug( 'three_ds_params=' . wp_json_encode( $three_ds_params ) );

			$transaction = $cbo_client->sale( $order, $card_number, $card_expiry, $card_cvc, $card_holder, $three_ds_params );
			CBOPAGA_Log::debug( 'Checkout data: ' . wp_json_encode( $transaction ) );
			...
			if ( 'authenticating' === ( $transaction['status'] ?? '' ) ) {
				...
				wc_add_notice( __( 'We were unable to complete the payment. Please contact with commerce.', 'class-cbopaga-payment-gateway' ), 'error' );
			} else {
				wc_add_notice( __( 'We were unable to complete the payment. Please check your card details or contact your bank.', 'class-cbopaga-payment-gateway' ), 'error' );

			}
		} catch ( \CBOPAGA_Exception $e ) {
			if ( ! $e->isSuccessResponse() ) {
				CBOPAGA_Log::debug( $e->getMessage() . ' - ' . wp_json_encode( $e->getResponse() ) );
				wc_add_notice( __( 'Cannot generate the payment. Please, contact with commerce.', 'class-cbopaga-payment-gateway' ), 'error' );
			} else {
				wc_add_notice( __( 'Cannot process the payment. Please, contact with commerce.', 'class-cbopaga-payment-gateway' ), 'error' );
			}
		}
	}
	...
	private function get3DSParams() {
		...
		$three_ds_params['billAddrState'] = ( '' !== $billing_state ) ? cbopaga_parse_state( $billing_state ) : 'DIG';
		...
		$three_ds_params['shipAddrState'] = ( '' !== $shipping_state ) ? cbopaga_parse_state( $shipping_state ) : 'DIG';
	...
	private function validate_payment( $transaction ) {
		...
		if ( in_array( $status, $success_status, true ) ) {
			$order->update_status( 'completed', __( 'Payment completed', 'class-cbopaga-payment-gateway' ) );
			...
			$order->update_status( 'failed', __( 'Failed payment', 'class-cbopaga-payment-gateway' ) );
		}
	...
	public function callback_url() {
		...
		wp_register_script( 'cbo-3ds-handler', '', array(), '1.0', true );
		wp_enqueue_script( 'cbo-3ds-handler' );
		...
		wp_add_inline_script( 'cbo-3ds-handler', $script_data, 'before' );
		...
		';
		wp_add_inline_script( 'cbo-3ds-handler', $main_script );
		...
		<head>
			...
			<title><?php esc_html_e( 'Processing 3DS…', 'class-cbopaga-payment-gateway' ); ?></title>
			<?php wp_head(); ?>
		</head>
		<body>
			<div class="cbo-3ds-loading">
				<div class="cbo-3ds-spinner"></div>
				<noscript>
					<p><?php esc_html_e( 'Please enable JavaScript to complete your payment. You will be automatically redirected...', 'class-cbopaga-payment-gateway' ); ?></p>
	...
	public function webhook() {
		...
		CBOPAGA_Log::debug( 'Webhook recibido: ' . wp_json_encode( $data ) );
	...
	public function get_iso_alpha3_cc( $country ) {
		return CBOPAGA_Constants::COUNTRIES[ $country ] ?? $country;
	}
